    ...
    <section id="speaking" style={{ padding: "5rem 0" }}>
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "0 1.5rem" }}>
        ...
        <motion.p
          ...
          style={{ textAlign: "center", color: "var(--text-secondary)", maxWidth: "600px", margin: "0 auto 3rem", fontSize: "1rem", fontStyle: "italic", fontFamily: "var(--font-serif)" }}
        >
        ...
        >
          {cards.map((card, i) => (
            <motion.article key={i} variants={fadeUp} style={{ padding: "2rem", border: "1px solid var(--border)", backgroundColor: "var(--bg-alt)" }}>
              <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", flexWrap: "wrap" as const, gap: "0.5rem", marginBottom: "0.3rem" }}>
                <h3 style={{ fontSize: "1.2rem", fontWeight: 600, margin: 0, color: "var(--text-primary)", fontFamily: "var(--font-serif)" }}>{card.role}</h3>
                <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem", textTransform: "uppercase" as const, letterSpacing: "0.05em", padding: "0.2rem 0.6rem", background: "var(--bg-primary)", color: "var(--text-secondary)", border: "1px solid var(--border)" }}>{card.date}</span>
              </div>
              <p style={{ fontSize: "0.95rem", color: "var(--machine-accent)", fontStyle: "italic", margin: "0 0 0.3rem 0" }}>{card.org}</p>
              <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", color: "var(--text-muted)", textTransform: "uppercase" as const, letterSpacing: "0.05em", marginBottom: "1.25rem" }}>{card.location}</p>
              <ul style={{ listStyle: "none", paddingLeft: 0, margin: 0 }}>
                {card.bullets.map((b, j) => (
                  <li key={j} style={{ position: "relative", paddingLeft: "1.5rem", marginBottom: "0.6rem", color: "var(--text-secondary)", fontSize: "0.95rem", lineHeight: 1.65 }}>
