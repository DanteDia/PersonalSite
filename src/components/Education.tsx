...
const EducationItem = ({ degree, school, detail }: { degree: string; school: string; detail: string }) => (
  <article style={{ padding: '1.5rem 0', borderBottom: '1px solid #e5e3df' }}>
    <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '0.3rem' }}>{degree}</h3>
    <span style={{ display: 'block', fontSize: '0.9rem', color: '#6a6a6a', fontStyle: 'italic', marginBottom: '0.5rem' }}>{school}</span>
    <p style={{ fontSize: '0.9rem', color: '#4a4a4a', margin: 0 }}>{detail}</p>
  </article>
);
...
export default function Education() {
  ...
  return (
    <section id="education" style={{ padding: '5rem 0', backgroundColor: '#f5f3ef', borderTop: '1px solid #e5e3df', borderBottom: '1px solid #e5e3df' }}>
